...
- [ ] 2.9 Add `CatalogClient` class to CAVEclient, wire to `CAVEclient.catalog` property (configured with datastack and auth token)
- [ ] 2.10 Add `register_asset()`, `validate_asset()`, `list_assets()`, `get_asset()`, `delete_asset()` methods (using mat_version/revision parameters)
- [ ] 2.11 Write client tests for Phase 0 methods against local server

## 3. Credential Vending + Client (Phase 1)
...
- [ ] 3.6 Add `get_access()` method to CatalogClient
- [ ] 3.7 Write client tests for credential vending against local server
- [ ] 3.8 (Future) Implement S3 credential backend: STS AssumeRole with inline prefix-scoped policy

## 4. View Definitions + Client (Phase 2)
...
- [ ] 4.4 Add `resolve_view()` and `to_duckdb_sql()` methods to CatalogClient
- [ ] 4.5 Write client tests for view resolution against local server
