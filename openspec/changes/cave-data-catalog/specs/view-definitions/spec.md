...
The system SHALL support assets with `asset_type: "view"`. A view asset SHALL have a `properties.definition` object containing: `dialect` (string, e.g., "sql"), `query` (string — SQL template with `{placeholder}` references to other assets), and `references` (object mapping placeholder names to asset reference paths in the format `datastack/name/mat_version/revision`). The special keyword `latest` MAY be used in place of `mat_version` and/or `revision` to indicate the reference should resolve to the highest available value at query time (e.g., `minnie65_public/synapses/latest/latest`).

#### Scenario: Registering a view with pinned references
- **WHEN** an authorized user registers a view with references like `{"synapses": "minnie65_public/synapses/943/1"}`
- **THEN** the system SHALL create the view asset and return 201 Created
...
The system SHALL provide `POST /api/v1/assets/{id}/resolve` for view assets. The endpoint SHALL resolve the view by: (1) resolving any `latest` keywords in references to concrete `mat_version` and `revision` values, (2) looking up each referenced asset, (3) vending credentials for each managed referenced asset, (4) substituting the placeholder names in the SQL template with format-appropriate scan expressions (e.g., `delta_scan('gs://...signed...')` for Delta tables), and (5) returning the resolved SQL string, all vended credentials, and the concrete resolved references (so the caller knows exactly which assets were used).
...
- **THEN** the system SHALL return 200 with a JSON body containing `resolved_query` (SQL with URIs substituted), `credentials` (array of per-asset credential objects), `dialect`, and `resolved_references` (mapping of placeholder names to concrete asset UUIDs and paths)
...
- **AND** the latest mat_version for synapses is 944 with revision 1
- **THEN** the system SHALL resolve to `minnie65_public/synapses/944/1` and return the resolved query with that asset's URI and credentials
